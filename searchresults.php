<?php
/*
This page selects a search condition based on what the user entered on the search page
and queries th database to return each matching result based off of it. Each search is specific
matching and the user must enter at least one field on the search form or they will be returned with an error.
...
//all possible conditions based on search form	
$searchquery = '';
if ($isbn != "" && $title =="" && $author == "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (isbn) AGAINST ('$isbn')");
	if(!$searchquery) die ("Couldn't access table(isbn) : " .mysql_error());
}
if ($isbn == "" && $title !="" && $author == "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Title) AGAINST ('$title')");
	if(!$searchquery) die ("Couldn't access table(title) : " .mysql_error());
}
if ($isbn == "" && $title =="" && $author != "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Author) AGAINST ('$author')");
	
	if(!$searchquery) die ("Couldn't access table(author) : " .mysql_error());
}
if ($isbn == "" && $title =="" && $author == "" && $class !="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Class) AGAINST ('$class')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn != "" && $title !="" && $author == "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (isbn, Title) AGAINST ('$isbn $title')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn != "" && $title =="" && $author != "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (isbn, Author) AGAINST ('$isbn $author')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn != "" && $title =="" && $author == "" && $class !="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (isbn, Class) AGAINST ('$isbn $class')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn == "" && $title !="" && $author != "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Title, Author) AGAINST ('$title $author')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn == "" && $title !="" && $author == "" && $class !="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Title, Class) AGAINST ('$title $class')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn == "" && $title =="" && $author != "" && $class !="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Author, Class) AGAINST ('$author $class')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn != "" && $title !="" && $author != "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (isbn, Title, Author) AGAINST ('$isbn $title $author')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn == "" && $title !="" && $author != "" && $class !="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (Title, Author, Class) AGAINST ('$title $author $class')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn != "" && $title !="" && $author != "" && $class !="")
{
	$searchquery = mysql_query("SELECT * FROM Books WHERE MATCH (isbn, Title, Author, Class) AGAINST ('$isbn $title $author $class')");
	if(!$searchquery) die ("Couldn't access table(class) : " .mysql_error());
}
if ($isbn == "" && $title =="" && $author == "" && $class =="")
{
	$searchquery = mysql_query("SELECT * FROM Books");
	if(!$searchquery) die ("Couldn't access table(empty) : " .mysql_error());
}
...
?>
Don't see the book your looking for? <a href = 'add' class='darker'>Add this book to your wishlist</a> to get notified when someone posts it.				
